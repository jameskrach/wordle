...
class BasePlayer(PlayerInterface):
    ...
    def remaining_letters(self) -> str:
        ...
        return "".join(i for i in ascii_lowercase if i not in bad_letters)
    ...
    def implied_regex(self) -> re.Pattern:
        ...
        pattern = "^" + "".join(in_pos.get(i, f"[{self.remaining_letters}]") for i in range(self.word_size)) + "$"
        return re.compile(pattern)

    @property
    def remaining_words(self) -> list[str]:
        """The remaining words that conform to previous responses."""
        return list(filter(self.implied_regex.fullmatch, self.all_words))
